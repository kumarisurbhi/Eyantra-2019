...
function jacobian_matrices = find_jacobian_matrices(eqbm_points, x1_dot, x2_dot)
  syms x1 x2
  solutions = {};             #cell array
  jacobian_matrices = {};     
  jac = [];
  for k = 1:length(eqbm_points)                #loop to iterate over eqbm_points
  ...
  ################## ADD YOUR CODE HERE ######################
  jac = jacobian([x1_dot,x2_dot], [x1,x2]);       #calculated jacobian matrix
  for k = 1:length(eqbm_points)                   #loop to find jacobian matrices
     tem = double(subs(jac, [x1 x2], solutions{k}));
     jacobian_matrices{k} = tem;
  endfor
  ############################################################  
endfunction
